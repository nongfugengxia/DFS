...
#define PROCESS_COUNT	10	// 需要模拟的客户端连接数目

typedef struct {
	int bytes;  //file size
	char *filename;
	int count;   //total file count 总共的文件数目
	int upload_count;	// 上传文件的数目(需要按大小呈正态分布)
	int success_count;  //success upload count 成功上传数目
	int fd;   //file description 文件描述
	int64_t time_used;  //unit: ms 所用时间：毫秒
...
static TestFileInfo files[FILE_TYPE_COUNT] = {
	{50 * 1024, "50K",        1000 / PROCESS_COUNT, 0, 0, -1, 0, NULL},
	{100 * 1024, "100K",      10000 / PROCESS_COUNT, 0, 0, -1, 0, NULL},
	{500 * 1024, "500K",     100000 / PROCESS_COUNT, 0, 0, -1, 0, NULL},
	{1 * 1024 * 1024, "1M",   100000 / PROCESS_COUNT, 0, 0, -1, 0, NULL},
	{10 * 1024 * 1024, "10M",  10000 / PROCESS_COUNT, 0, 0, -1, 0, NULL},
...
/*
 * 第一个参数为上传文件的线程个数 proccess_index = atoi(argv[1]);
 * 第二个参数为配置文件的路径 conf_filename = argv[2];
 */
int main(int argc, char **argv)
{
	...
	{
		char currentWorkDirBuff[80];
		getcwd(currentWorkDirBuff, sizeof(currentWorkDirBuff));
		strcat(currentWorkDirBuff, "/test");
		chdir(currentWorkDirBuff);		// 将当前工作路径从工程根目录下改变到/test目录下
		strcat(currentWorkDirBuff, "/client.conf");
		printf("FastDFS客户端配置文件路径：%s\n", currentWorkDirBuff);
		conf_filename = currentWorkDirBuff;
//		conf_filename = "./client.conf";
	}

	if ((result = load_file_contents()) != 0)	// 读取需要上传的文件
	{
		return result;
	}
	...
	total_count = 0;
	success_count = 0;
	while (total_count < upload_count)	// upload_count为需要上传文件的总数
	{
		rand_num = (int)(upload_count * ((double)rand() / RAND_MAX));		// 产生一个1~upload_count之间的随机数字
		for (file_index=0; file_index<FILE_TYPE_COUNT; file_index++)
		{
			if (rand_num < count_sums[file_index])							// 随机选择第file_index类文件进行上传
			{
				break;
			}
		}

		if (files[file_index].upload_count >= files[file_index].count)		// 任意一种类型文件 的上传数目不能大于文件的总数目
		{
			continue;
		}

		files[file_index].upload_count++;		// 进行第file_index种文件上传,它的上传数目就要+1
		total_count++;							// 所有文件上传数+1
...
/*
 * 查看进程信息：	pgrep test_upload | xargs ps -u --pid
 * 杀死进程：		kill -s 9 3260
 *
 * error while loading shared libraries: 在run configuration --> Environment里面添加LD_LIBRARY_PATH环境变量
 */
