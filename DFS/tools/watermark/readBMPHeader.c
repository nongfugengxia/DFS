...
bmp readbmpfile(void) {
	bmp m;        //定义一个位图结构
	FILE *fp;
	if ((fp = fopen("/home/ubuntu/install/eclipse/workspace/DFS/lmk.bmp", "r")) == NULL) {
		printf("can't open the bmp imgae.\n ");
		exit(0);
	} else {
		unsigned char *bmphead;
		bmphead = malloc(54 + 256 * 4 * sizeof(unsigned char));
		char* in = "";
		fread(bmphead, 54 + 256 * 4 * sizeof(unsigned char), 1, fp);
		fclose(fp);

		FILE *pWriteFile = fopen("/home/ubuntu/install/eclipse/workspace/DFS/bmp256.HD", "wb");
		fwrite (bmphead , sizeof(char), 54 + 256 * 4 * sizeof(unsigned char), pWriteFile);
		fclose(pWriteFile);
		printf("readed\n");
